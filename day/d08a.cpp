...
vector<int> circuit_rank(DSU circuit, int point_count) {
    bool temp[1000] = {};
    vector<int> output;
    for (int i = 0; i < point_count; i++) {
        int pi = circuit.parent(i);
        if (!temp[pi]) {
            temp[pi] = true;
            output.push_back(circuit.setSize(i));
        }
    }
    return output;
}
...
int main(){
    ...
    vector<int> ranking = circuit_rank(circuit, n);
    sort(ranking.begin(), ranking.end());
    reverse(ranking.begin(), ranking.end());
    cout << ranking[0] * ranking[1] * ranking[2];
