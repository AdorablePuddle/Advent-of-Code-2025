...
// Preprocess the sequences of digits repeated at least twice.
...
void multiplying(long long int v, long long int limit, vector<long long int> &output) {
    string value = to_string(v);
    string current = value + value;
    while (stoll(current) < limit) {
        output.push_back(stoll(current));
...
long long int total_multiple(string input, vector<long long int> list) {
    ...
    for (int i = p_start; i <= p_end; i += 1) {
        if (list[i] != prev) // This is one stupid way to bypass the fact that the list have multiple copies :D
            total += list[i];
        prev = list[i];
    }
    return total;
...
// Debug
void printPreprocess(vector <long long int> vl) {
    for (long long int v : vl) {
        cout << v << "\n";
    }
}
...
int main() {
    ...
    for (string segment : segments) {
        final_total += total_multiple(segment, doubleList);
    }
    cout << final_total;
}