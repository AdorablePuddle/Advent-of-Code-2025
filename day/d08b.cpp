...
int main(){
    ...
    int connection_limit = n - 1, last_edge = 0;
    DSU circuit(n);

    for (int i = 0; i < (int)edge_list.size() && connection_limit; i++) {
        if (!circuit.isSameSet(edge_list[i].second.first, edge_list[i].second.second)) {
    ...
    cout << point_list[edge_list[last_edge].second.first].x * point_list[edge_list[last_edge].second.second].x;
}